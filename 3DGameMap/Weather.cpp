...
int CFlyFlower::GetY(int nx)
{
	...
	{
	case 0:
		nRet = 4E-15*nx*nx*nx*nx*nx*nx - 7E-12*nx*nx*nx*nx*nx + 5E-09*nx*nx*nx*nx - (3E-06 + m_nA /100)*nx*nx*nx + (0.0014 + m_nB /100)*nx*nx - 0.3874 *nx + 121.46;
		break;	
	case 1:
		nx = nx/3;
		nRet =  4E-14*nx*nx*nx*nx*nx*nx - 1E-10*nx*nx*nx*nx*nx + 1E-07*nx*nx*nx*nx - (7E-05 + m_nA /100)*nx*nx*nx + (0.016 + m_nB /100)*nx*nx - 1.5028*nx + 112.04;
		break;
	case 2:
		nRet = -3E-15 *nx*nx*nx*nx*nx*nx + 9E-12*nx*nx*nx*nx*nx - 9E-09*nx*nx*nx*nx + (5E-06+ m_nA /100)*nx*nx*nx - (0.0008 + m_nB /100)*nx*nx - 0.1221*nx + 124.15;
		break;
	case 3:
		nRet = -9E-15*nx*nx*nx*nx*nx*nx + 1E-11*nx*nx*nx*nx*nx + 1E-09*nx*nx*nx*nx - (8E-06+ m_nA /100)*nx*nx*nx + (0.0035 + m_nB /100)*nx*nx - 0.52*nx + 103.45;
		break;
	case 4:
		nRet = -2E-14*nx*nx*nx*nx*nx*nx + 4E-11*nx*nx*nx*nx*nx - 4E-08*nx*nx*nx*nx + (2E-05 + m_nA /100)*nx*nx*nx - (0.0022 + m_nB /100)*nx*nx - 0.1433*nx + 118.39;
		break;
	case 5:
		nRet = -1E-15*nx*nx*nx*nx*nx*nx + 4E-12*nx*nx*nx*nx*nx - 3E-09*nx*nx*nx*nx + (1E-06 + m_nA /100)*nx*nx*nx + (1E-04 + m_nB /100)*nx*nx - 0.0116*nx + 8.669;
		break;
	case 6:
		nRet = 7E-15*nx*nx*nx*nx*nx*nx - 2E-11*nx*nx*nx*nx*nx + 2E-08*nx*nx*nx*nx - (1E-05 + m_nA /100)*nx*nx*nx + (0.0031 + m_nB /100)*nx*nx - 0.479*nx + 122.14;
		break;
	case 7:
		nx = nx/2;
		nRet =  -5E-14*nx*nx*nx*nx*nx*nx + 1E-10*nx*nx*nx*nx*nx - 1E-07*nx*nx*nx*nx + (4E-05 + m_nA /100)*nx*nx*nx - (0.003 + m_nB /100)*nx*nx - 0.6211*nx + 131.81;
		break;
	case 8:
		nRet = -9E-16*nx*nx*nx*nx*nx*nx + 3E-12*nx*nx*nx*nx*nx - 4E-09*nx*nx*nx*nx + (2E-06+ m_nA /100)*nx*nx*nx - (0.0001+ m_nB /100)*nx*nx - 0.0838*nx + 11.637;
		break;
		
	case 9:
		nRet = 3E-15 * nx*nx*nx*nx*nx*nx - 8E-12 * nx*nx*nx*nx*nx + 6E-09 * nx*nx*nx*nx - (2E-06 + m_nA /100)* nx*nx*nx + (0.0005+ m_nB /100)* nx*nx - 0.2155*nx + 125.61;
		break;
...
CSysFlyFlower::Create( int nNum, int nType,DWORD dwDelayTime)
{
	Destroy();
	m_nType = nType;
	char szTitle[256];
...
void CSysFlyFlower::Destroy()
{
	if(	m_iStatu == _FLYFLOWERSTATUS_NONE)
		return; 
	int i;	
	for(i= 0;i< m_dequeFlyFlower.size();i++)	 
