...
void CDataControlAni::Show(int nFrame,int nCurAction,int nX,int nY,int nAlpha)
{
	...
	if(nCurAction > m_dequeAction.size())
...
int  CDataControlAni::GetControlFrame(int nAction)//The total number of frames to be NACTION
{
...
{
	for(int i =0;i<m_dequeAction.size();i++)
	{
		if(m_dequeAction[i])
		{
