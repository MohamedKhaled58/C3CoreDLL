...
//Several required special treatment performance status
const __int64	PLAYERTATUS_ATTACK			=0x00010000;			// Attack upgrade
const __int64	PLAYERTATUS_ADJUST_XP		=0x0000010000000000;	// Increase in the value of each adjustment XP
...
BOOL	C3DRole::Draw ( int nMapX, int nMapY )
{
	...
	// If there is copy body copy body only draw
	if(m_pArmorClone)
	{
	...
	else
	{
		if (m_pHead)
		{
			// Helmets to follow the shared bone
			m_pHead->SetTexture();
...
/*		
	...
	// If there is copy body copy body draw only alpha
	if(m_pArmorClone)
	{
		//m_pArmorClone->SetTexture();
		m_pArmorClone->DrawAlpha ( m_nShadowType, m_LightOffset.x, m_LightOffset.y, m_LightOffset.z, m_a, m_r, m_g, m_b, m_nPosH );
