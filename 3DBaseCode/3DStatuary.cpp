...
int C3DStatuary::SetAction(int nActionType, BOOL bResetMotion, BOOL bPlaySound, int nWorldX, int nWorldY, BOOL bSetEffect)
{
	...
	if (m_pLWeapon)
	{
		...
		// In order to speed up the pace to do something to judge
		// Only left-handed weapons are bow-type weapons Motion
...
BOOL C3DStatuary::Draw ( int nMapX, int nMapY )
{
	...
	// If there is copy body copy body painting only
	if(m_pArmorClone)
	{
	...
	// If there is copy body copy body painting only alpha
	if(m_pArmorClone)
	{
		m_pArmorClone->SetTexture();
